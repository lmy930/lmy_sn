...
generate_spatial_data_ar1_1cp <- function(n=1000, p=2, K, cp_loc=500, shift_size=2.5, anomaly_only=TRUE) {
  ...
  for (i in stations_to_change) {
    data_list[[i]][cp_loc:n, ] <- sweep(data_list[[i]][cp_loc:n, , drop=FALSE], 2, rep(shift_size, p), "+")
  }
  return(data_list)
}
...
run_case_logic <- function(condition, coords, n=1000, p=2, cp_true=500, L=50, M=1000, block_size=15) {
  data_list <- generate_spatial_data_ar1_1cp(n, p, nrow(coords), cp_true, 2.5, condition == "Anomaly")
...
run_monte_carlo_1cp <- function(N_sim = 50, raw_coords) {
  ...
  for (cond in conditions) {
    ...
    for (iter in 1:N_sim) {
      res <- run_case_logic(cond, coords, n=1000, cp_true=500)
