...
generate_spatial_data_ar1_2cp <- function(n=1000, p=2, K, cp_locs=c(333, 666), shift_sizes=c(2.5, -2.5), anomaly_only=TRUE) {
  ...
  stations_to_change <- if (anomaly_only) 1 else seq_len(K)
  for (i in stations_to_change) {
  
    data_list[[i]][cp_locs[1]:(cp_locs[2]-1), ] <- sweep(data_list[[i]][cp_locs[1]:(cp_locs[2]-1), , drop=FALSE], 2, rep(shift_sizes[1], p), "+")
    
   
    net_shift <- shift_sizes[1] + shift_sizes[2] 
    data_list[[i]][cp_locs[2]:n, ] <- sweep(data_list[[i]][cp_locs[2]:n, , drop=FALSE], 2, rep(net_shift, p), "+")
  }
  return(data_list)
}
...
evaluate_metrics_multi <- function(n, cp_true, detected_cps, tol = 50, condition) {
  ...
  true_labels <- get_labels(n, if(condition == "Anomaly") cp_true else c())
  ...
  if (condition == "Anomaly") {
    if (length(detected_cps) > 0) {
      ...
      min_dists_det <- apply(dist_mat, 1, min) 
      min_dists_true <- apply(dist_mat, 2, min) 
      
      TP <- sum(min_dists_det <= tol)
      FP <- length(detected_cps) - TP
      MAE <- mean(min_dists_true) 
      H_dist <- max(max(min_dists_det), max(min_dists_true)) 
      
      Recall <- sum(min_dists_true <= tol) / length(cp_true)
      Precision <- ifelse(length(detected_cps) == 0, 0, TP / length(detected_cps))
    } else {
...
run_case_logic <- function(condition, coords, n=1000, p=2, cp_true=c(333, 666), L=50, M=1000, block_size=15) {
  ...
  data_list <- generate_spatial_data_ar1_2cp(n, p, nrow(coords), cp_true, shift_sizes=c(2.5, -2.5), anomaly_only=(condition == "Anomaly"))
  ...
  metrics <- evaluate_metrics_multi(n, cp_true, final_detected_cps, tol = 50, condition)
  
  if (condition == "Anomaly") {
    score <- (metrics$F1 * 1000) - (ifelse(is.na(metrics$MAE), 500, metrics$MAE)) - (metrics$FP * 50)
  } else {
...
generate_best_plots <- function(best_run, condition, cp_true, L, M, block_size, coords, prefix, n, p) {
  ...
  png(filename = sprintf("%s_%s_TimeSeries.png", prefix, condition), width = 1200, height = 800, res = 120)
  ...
  for(j in 1:p) {
    ...
    if(condition == "Anomaly") abline(v=cp_true, col="gray40", lty=3, lwd=2)
  ...
  if(!is.null(scan_df)) {
    plot(scan_df$tau, scan_df$S_obs, type="l", col="blue", main="SBB Statistic Trend (2 CPs)", ylab="S_obs", lwd=1.5)
...
run_monte_carlo_2cp <- function(N_sim = 50, raw_coords) {
  ...
  for (cond in conditions) {
    ...
    for (iter in 1:N_sim) {
      res <- run_case_logic(cond, coords, n=1000, cp_true=true_cps)
      
 
      if(cond == "Anomaly") {
        
        if(res$metrics$Recall == 1 && res$metrics$FP == 0) correct_count <- correct_count + 1
    ...
    summary_df <- rbind(summary_df, data.frame(
      Condition = cond,
      Strict_Acc = round(correct_count / N_sim, 3), 
      Avg_FP = round(total_fp / N_sim, 3),
      F1_Score = ifelse(cond == "Anomaly", round(mean(all_F1, na.rm=TRUE), 3), NA),
      Avg_MAE = ifelse(cond == "Anomaly", round(mean(all_MAE, na.rm=TRUE), 2), NA),
      Hausdorff = ifelse(cond == "Anomaly", round(mean(all_Hausdorff, na.rm=TRUE), 2), NA),
