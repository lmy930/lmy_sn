...
generate_best_plots <- function(best_run, condition, cp_true, L, M, block_size, coords, prefix, n, p) {
  ...
  # ==========================================
  # 强制转换为纯数值，彻底剥离原有的 tibble/matrix 隐藏列名属性
  ...
  # 为了兼容低版本的 ggplot2，这里不仅定义 linewidth，还可以用做 size
  custom_linewidths <- c(
  ...
  plot_ts <- function(data, title, y_label, cp_color) {
    p_base <- ggplot(data, aes(x = Time, y = Value, color = Group, linetype = Group, shape = Group)) +
      # 使用 size 替代 linewidth 以获得更好的全版本兼容性
...
set.seed(2026)
mc_results <- run_real_data_eval(data_dir = "2025_new", N_sim = 10, raw_coords = raw_station_coords)