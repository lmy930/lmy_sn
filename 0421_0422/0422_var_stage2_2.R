...
# 3. 稳健修改点：2CP 方差数据生成
# ==========================================
generate_spatial_data_ar1_2cp_var <- function(n=1000, p=2, K, cp_locs=c(333, 666), var_scale=2.5, condition="Anomaly_Both") {
  ...
  for (i in seq_len(K)) {
    ...
    for (t in 2:n) {
      ...
      # 偏移量作用于[cp1, cp2-1] 区间（只缩放白噪声，保持自相关结构）
      if (t >= cp_locs[1] && t < cp_locs[2]) {
        # 变量 1 的波动放大
...
# ==========================================
# 4. 稳健修改点：绝对偏差映射（Absolute Deviation Mapping）
# ==========================================
...
run_case_logic_var <- function(condition, coords, n=1000, p=2, cp_true=c(333, 666), L=50, M=1000, block_size=15) {
  data_list <- generate_spatial_data_ar1_2cp_var(n, p, nrow(coords), cp_true, 2.5, condition)
  
  # 修改点：Stage 1 改为 "covariance"
...
#   if(!is.null(scan_df)) {
#     plot(scan_df$tau, scan_df$S_obs, type="l", col="blue", main="Robust Absolute-Deviation SBB Statistic Trend", ylab="S_obs", lwd=1.5)
...
generate_best_plots_var <- function(best_run, condition, cp_true, L, M, block_size, coords, prefix, n, p) {
  ...
  if(!is.null(scan_df)) {
    p_stat <- ggplot(scan_df, aes(x = tau)) +
      ...
      # 此处已更新为你代码中要求的标题
      labs(title = "Robust Absolute-Deviation SBB Statistic Trend", x = NULL, y = "S_obs") +
      theme_minimal(base_size = 14) +
...
run_monte_carlo_2cp_var <- function(N_sim = 50, raw_coords) {
  ...
  for (cond in conditions) {
    ...
    for (iter in 1:N_sim) {
      res <- run_case_logic_var(cond, coords, n=1000, p=2, cp_true=true_cps, L=50, M=1000, block_size=15)
    ...
    generate_best_plots_var(best_run, cond, cp_true=true_cps, L=50, M=1000, block_size=15, coords=coords, prefix="BestRun_AR1_2CP", n=1000, p=2)
...
set.seed(2026)
mc_results <- run_monte_carlo_2cp_var(N_sim = 5, raw_coords = raw_station_coords)
...
generate_spatial_data_ar1_2cp_var <- function(n=1000, p=2, K, cp_locs=c(333, 666), var_scale=2.5, condition="Anomaly_Both") {
  ...
  for (i in seq_len(K)) {
    ...
    for (t in 2:n) {
      noise <- as.numeric(rmvnorm(1, mean = rep(0, p), sigma = Sigma_eps))
      scale_vec <- c(1, 1) # 默认正常波动
      
      # 双变点逻辑：仅在 cp_locs[1] 到 cp_locs[2] 的区间内发生方差突增
      if (t >= cp_locs[1] && t < cp_locs[2]) {
        # Var1 异常逻辑
...
# 修改点：多变点评估逻辑
evaluate_metrics_2cp <- function(n, cp_true, detected_cps, tol = 50, condition) {
  is_anomaly_cond <- condition != "Normal" 
  
  # 用于计算ARI的标签生成器 (支持多变点分段)
  get_labels <- function(n, cps) {
    lbls <- rep(1, n)
    if(length(cps) == 0) return(lbls)
    cps <- sort(cps[cps > 0 & cps < n])
  ...
  if (is_anomaly_cond) {
    dist_mat <- abs(outer(detected_cps, cp_true, "-")) # 自动处理多个真实变点
    if (length(detected_cps) > 0) {
      min_dists <- apply(dist_mat, 2, min) # 每个真实变点距离最近的检测变点的距离
      TP <- sum(min_dists <= tol)
      FP <- length(detected_cps) - TP
      MAE <- mean(min_dists) # 多个变点时取平均MAE
      H_dist <- max(max(min_dists), max(apply(dist_mat, 1, min)))
    } else {
      TP <- 0; FP <- 0; MAE <- NA; H_dist <- n
    }
    ...
    Precision <- ifelse(TP + FP == 0, 0, TP / (TP + FP))
    Recall <- TP / length(cp_true) # 除以真实变点总数 (双变点时为2)
    F1 <- ifelse(Precision + Recall == 0, 0, 2 * Precision * Recall / (Precision + Recall))
...
run_case_logic_2cp_var <- function(condition, coords, n=1000, p=2, cp_true=c(333, 666), L=50, M=1000, block_size=15) {
  data_list <- generate_spatial_data_ar1_2cp_var(n, p, nrow(coords), cp_true, 2.5, condition)
  
  # Stage 1: 扫描 "covariance"
  sn_res <- SNSeg_Multi(data_list[[1]], paras_to_test = "covariance", confidence = 0.95, grid_size_scale = 0.05, plot_SN = FALSE)
  ...
  metrics <- evaluate_metrics_2cp(n, cp_true, final_detected_cps, tol = 50, condition)
  
  # 修改点：双变点打分机制，鼓励找到所有的TP
  if (condition != "Normal") {
    score <- if (metrics$TP > 0) (metrics$TP * 1000 - (if(!is.na(metrics$MAE)) metrics$MAE else 0) - metrics$FP * 200) else (-5000 - metrics$FP * 200)
  } else {
    score <- -metrics$FP * 100
  }
...
# ==========================================
# 5. 可视化与模拟模块 (支持多个竖线的绘制)
# ==========================================
generate_best_plots_2cp_var <- function(best_run, condition, cp_true, L, M, block_size, coords, prefix, n, p) {
  data_list <- best_run$data_list
  detected_cps <- best_run$cps
  ...
  png(filename = sprintf("%s_%s_2CP_Variance_TimeSeries.png", prefix, condition), width = 1200, height = 1000, res = 120)
  par(mfrow=c(p + 1, 1), mar=c(4, 4, 2, 1)) 
  
  for(j in 1:p) {
    plot(1:n, target[,j], type="l", col="#d7301f", main=sprintf("Var %d Time Series (2 CPs Variance | %s)", j, condition), ylab="Value", ylim=range(c(target[,j], ref[,j])))
    lines(1:n, ref[,j], col="#2171b5", lty=2)
  ...
  png(filename = sprintf("%s_%s_2CP_Variance_Statistics.png", prefix, condition), width = 1200, height = 800, res = 120)
  par(mfrow=c(2, 1), mar=c(4, 4, 2, 1))
  if(!is.null(scan_df)) {
  ...
  dev.off()
  cat(sprintf("\n=> 最佳运行图表已分离保存: [%s] 和[%s]\n", 
              sprintf("%s_%s_2CP_Variance_TimeSeries.png", prefix, condition), 
              sprintf("%s_%s_2CP_Variance_Statistics.png", prefix, condition)))
}
...
run_monte_carlo_2cp_var <- function(N_sim = 50, raw_coords) {
  ...
  coords <- find_and_reorder_stations(raw_coords)
  summary_df <- data.frame()
  cp_true_setup <- c(333, 666) # 设定两个变点的位置
  
  for (cond in conditions) {
    cat(sprintf("\n=== 正在运行: 2 CP VARIANCE | 场景: %s ===\n", toupper(cond)))
    ...
    pb <- txtProgressBar(min = 0, max = N_sim, style = 3)
    for (iter in 1:N_sim) {
      res <- run_case_logic_2cp_var(cond, coords, n=1000, p=2, cp_true=cp_true_setup, L=50, M=1000, block_size=15)
      
      is_anomaly_cond <- cond != "Normal"
      
      if(is_anomaly_cond) {
        if(res$metrics$TP == 2) correct_count <- correct_count + 1 # 双变点都找到才算完美命中
        all_F1 <- c(all_F1, res$metrics$F1)
        if(!is.na(res$metrics$MAE)) all_MAE <- c(all_MAE, res$metrics$MAE)
        all_Hausdorff <- c(all_Hausdorff, res$metrics$Hausdorff)
      } else {
    ...
    generate_best_plots_2cp_var(best_run, cond, cp_true=cp_true_setup, L=50, M=1000, block_size=15, coords=coords, prefix="BestRun_2CP", n=1000, p=2)
    
    summary_df <- rbind(summary_df, data.frame(
      Condition = cond,
      Exact_Match_Rate = round(correct_count / N_sim, 3), # 双变点全命中的比例
      Avg_FP = round(total_fp / N_sim, 3),
      F1_Score = ifelse(cond != "Normal", round(mean(all_F1, na.rm=TRUE), 3), NA),
      Avg_MAE = ifelse(cond != "Normal", round(mean(all_MAE, na.rm=TRUE), 2), NA),
      Hausdorff = ifelse(cond != "Normal", round(mean(all_Hausdorff, na.rm=TRUE), 2), NA),
...
# N_sim设置为2用于快速测试跑通，正式实验建议设置为50-100
set.seed(2026)
mc_results_2cp_var <- run_monte_carlo_2cp_var(N_sim = 5, raw_coords = raw_station_coords)