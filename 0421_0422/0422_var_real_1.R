...
# ==========================================
# 3. 路线一：独立加性噪声 + 绝对值反弹 (全新调整)
# ==========================================
generate_spatial_data_real_1cp_var <- function(base_data_list, cp_loc, noise_sd_1, noise_sd_2, condition) {
  ...
  for (i in seq_len(K)) {
    if (cp_loc <= n) {
      ...
      # 生成独立的加性高斯白噪声
      noise_v1 <- rnorm(len_cp, mean = 0, sd = noise_sd_1)
      noise_v2 <- rnorm(len_cp, mean = 0, sd = noise_sd_2)
      
      # 靶向站点的噪声注入逻辑
      if ((condition == "Anomaly_Both" && i == 1) || (condition == "Normal") ||
          (condition == "Anomaly_Var1" && i == 1) || (condition == "Anomaly_Var2")) {
        data_list[[i]][cp_loc:n, 1] <- data_list[[i]][cp_loc:n, 1] + noise_v1
      }
      
      if ((condition == "Anomaly_Both" && i == 1) || (condition == "Normal") ||
          (condition == "Anomaly_Var1") || (condition == "Anomaly_Var2" && i == 1)) {
        data_list[[i]][cp_loc:n, 2] <- data_list[[i]][cp_loc:n, 2] + noise_v2
      }
      
      # 💡 核心修改：使用“绝对值反弹”代替原来的“截断为0”，完美保留方差波动！
      data_list[[i]][, 1] <- abs(data_list[[i]][, 1])
      data_list[[i]][, 2] <- abs(data_list[[i]][, 2])
    }
  }
  return(data_list)
}
...
build_feature_matrix_var <- function(data_list, tau, L, coords) {
  ...
  if (actual_L < 6 || (e_idx - s_idx + 1) != 2 * actual_L) return(NULL)
...
# 5. 修复奇异矩阵的工作流核心逻辑
# ==========================================
run_case_logic_real_var <- function(condition, base_data_list, coords, cp_true, noise_sd_1, noise_sd_2, L=12, M=500, block_size=4) {
  n <- nrow(base_data_list[[1]])
  p <- ncol(base_data_list[[1]])
  
  data_list <- generate_spatial_data_real_1cp_var(base_data_list, cp_true, noise_sd_1, noise_sd_2, condition)
  
  target_mat <- data_list[[1]]
  
  # 1. 强制标准化：极大改善协方差矩阵的数值稳定性和条件数 (仅供 Stage 1)
  target_scaled <- scale(target_mat)
  
  # 2. 注入稍大一点的白噪声：sd = 0.01 防止长平线或共线导致奇异
  jitter_mat <- matrix(rnorm(n * p, mean = 0, sd = 1e-2), nrow = n, ncol = p)
  target_ready_for_snseg <- target_scaled + jitter_mat
  
  # Stage 1: 使用加了扰动和标准化的数据进行初步扫描
  sn_res <- tryCatch({
    SNSeg_Multi(target_ready_for_snseg, paras_to_test = "covariance", confidence = 0.95, grid_size_scale = 0.05, plot_SN = FALSE)
  }, error = function(e) {
    cat(sprintf("\n [警告] SNSeg 内部发生奇异错误，跳过粗筛: %s\n", e$message))
    return(list(est_cp = c()))
  })
  
  candidates <- list()
  for (ecp in sn_res$est_cp) {
    # 💡 核心：Stage 2 的严格检验依然使用【原始、干净】的 data_list
...
# 6. 美化版绘图函数
# ==========================================
# generate_best_plots_real_var <- function(best_run, condition, cp_true, L, M, block_size, coords, prefix, n, p) {
...
#   png(filename = sprintf("%s_%s_Variance_TimeSeries.png", prefix, condition), width = 1200, height = 1000, res = 150)
#   par(mfrow=c(p + 1, 1), mar=c(4, 4.5, 2.5, 1), cex.main=1.2, cex.lab=1.1) 
  
#   for(j in 1:p) {
#     plot(1:n, target[,j], type="o", pch=20, cex=0.8, lwd=2, col="#d7301f", 
#          main=sprintf("%s Time Series (Variance Additive | %s)", var_names[j], condition), 
...
#   png(filename = sprintf("%s_%s_Variance_Statistics.png", prefix, condition), width = 1200, height = 800, res = 150)
...
generate_best_plots_real_var <- function(best_run, condition, cp_true, L, M, block_size, coords, prefix, n, p) {
  ...
  # ==========================================
  # 1. 实际数据时序对齐与长格式转换 (修复变灰 Bug)
  # ==========================================
  # 强制转换为纯数值，彻底剥离原有的 tibble/matrix 隐藏属性
  ...
  # 还原原代码的 pch=20(实心圆) 和 pch=18(实心菱形)
  custom_shapes <- c(
    "PM2.5 - Target" = 16, "PM2.5 - Ref" = 16,
    "PM10 - Target"  = 18, "PM10 - Ref"  = 18 
  )
  # 使用 size 替代 linewidth 控制线条粗细
  custom_linewidths <- c(
  ...
  plot_ts <- function(data, title, y_label, cp_color) {
    p_base <- ggplot(data, aes(x = Time, y = Value, color = Group, linetype = Group, shape = Group)) +
      ...
      {if(condition != "Normal") geom_vline(xintercept = cp_true, color = "gray40", linetype = "dotted", size = 1.2)} +
  ...
  # 注意这里的标题加入了 (Variance Additive | %s)
  p1 <- plot_ts(df_long %>% filter(Variable == "PM2.5"), sprintf("PM2.5 Time Series (Variance Additive | %s)", condition), "PM2.5 (ug/m3)", cp_col1)
  p2 <- plot_ts(df_long %>% filter(Variable == "PM10"), sprintf("PM10 Time Series (Variance Additive | %s)", condition), "PM10 (ug/m3)", cp_col2)
  p3 <- plot_ts(df_long, sprintf("Combined View: PM2.5 & PM10 (%s)", condition), "Concentration", cp_col_combined) +
        labs(x = "Time Index") +
        theme(legend.position = "bottom")
  
  # 拼图并保存 (Variance_TimeSeries)
...
# 7. 主程序入口
# ==========================================
run_real_data_eval_var <- function(data_dir, N_sim = 5, raw_coords) {
  base_data_list <- load_real_data(data_dir)
  n_len <- nrow(base_data_list[[1]])
  
  cp_true <- floor(n_len / 2) 
  
  # 💡 为两个变量分别设定强烈的加性噪声标准差，确保冲破低基数底噪
  noise_sd_v1 <- 15  # PM2.5 在平缓期本底约在 5 以内，15 的标准差视觉冲击极强
  noise_sd_v2 <- 40  # PM10 在平缓期本底约在 30 左右，40 的标准差足够明显
  ...
  for (cond in conditions) {
    ...
    for (iter in 1:N_sim) {
      res <- run_case_logic_real_var(cond, base_data_list, coords, cp_true=cp_true, 
                                     noise_sd_1=noise_sd_v1, noise_sd_2=noise_sd_v2, 
                                     L=12, M=500, block_size=4)
    ...
    generate_best_plots_real_var(best_run, cond, cp_true=cp_true, L=12, M=500, block_size=4, 
                                 coords=coords, prefix="BestRun_RealData_VAR_1CP", n=n_len, p=2)
...
set.seed(2026)
# 文件夹名称请根据你本地的实际存放路径进行调整
mc_results_real_var <- run_real_data_eval_var(data_dir = "2025_new", N_sim = 30, raw_coords = raw_station_coords)