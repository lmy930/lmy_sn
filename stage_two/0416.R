...
detect_cp_stage1 <- function(target_mat, scenario) {
  if (scenario == "mean") {
    SNSeg_Multi(target_mat, paras_to_test = "mean", confidence = 0.95, grid_size_scale = 0.05, grid_size = 25, plot_SN = FALSE)
  } else if (scenario == "cov") {
    SNSeg_Multi(target_mat, paras_to_test = "covariance", confidence = 0.95, grid_size_scale = 0.05, grid_size = 25, plot_SN = FALSE)
  } else {
    SNSeg_Multi(target_mat, paras_to_test = "mean", confidence = 0.95, grid_size_scale = 0.05, grid_size = 25, plot_SN = FALSE)
  }
}
...
run_case_4cp <- function(scenario = "mean", condition = "Anomaly", coords, n = 1000, p = 2, 
                         ...
                         alpha = 0.05, L = 50, M = 500, bandwidth = 50, # 注意：L被调整为50以避免交叉污染
