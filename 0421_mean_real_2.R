...
# 核心修改：生成双变点(区间异常)，注入真实数据基底
generate_spatial_data_real_2cp <- function(base_data_list, cp_locs, shift_size, condition) {
  data_list <- base_data_list
  K <- length(data_list)
  
  for (i in seq_len(K)) {
    # 变量 1：偏移作用于 [cp1, cp2-1] 区间
...
run_case_logic <- function(condition, base_data_list, coords, cp_true, shift_size, L=12, M=1000, block_size=4) {
  ...
  data_list <- generate_spatial_data_real_2cp(base_data_list, cp_true, shift_size, condition)
  ...
  for (ecp in sn_res$est_cp) {
    ...
    # 核心修改：使用严格显著性水平 p <= 0.01 对抗实际数据的天然噪音
    if (!is.null(stage2) && stage2$p_value <= 0.01) {
      # 允许多个变点，只需保证彼此之间的距离大于容差 L 即可
      if (!any(abs(final_detected_cps - ecp) <= L)) {
...
#     grid(col = "gray85", lty = 1)
#     # 统计显著性阈值线也调整为了更严谨的 0.01
#     abline(h=0.01, col="#f16913", lty=2, lwd=2)
#     if(condition != "Normal") abline(v=cp_true, col="gray40", lty=3, lwd=3)
#     if(length(detected_cps) > 0) abline(v=detected_cps, col="#2ca25f", lty=1, lwd=3)
#   }
#   dev.off()
...
generate_best_plots <- function(best_run, condition, cp_true, L, M, block_size, coords, prefix, n, p) {
  ...
  # 使用 size 替代 linewidth 控制线条粗细，兼顾版本兼容性
  custom_linewidths <- c(
  ...
  # 绘制三张子图
  p1 <- plot_ts(df_long %>% filter(Variable == "PM2.5"), sprintf("PM2.5 Time Series (%s)", condition), "PM2.5 (ug/m3)", cp_col1)
  p2 <- plot_ts(df_long %>% filter(Variable == "PM10"), sprintf("PM10 Time Series (%s)", condition), "PM10 (ug/m3)", cp_col2)
  ...
  if(!is.null(scan_df)) {
    p_stat <- ggplot(scan_df, aes(x = tau)) +
      ...
      labs(title = "SBB Statistic Trend", x = "Time Index", y = "S_obs") +
      theme_minimal(base_size = 14) +
    ...
    p_pval <- ggplot(scan_df, aes(x = tau)) +
      ...
      # 核心修改：遵循你代码的设定，将统计显著性 P-value 阈值线改为 0.01
      geom_hline(yintercept = 0.01, color = "#f16913", linetype = "dashed", size = 1.0) +
      {if(condition != "Normal") geom_vline(xintercept = cp_true, color = "gray40", linetype = "dotted", size = 1.2)} +
      {if(length(detected_cps) > 0) geom_vline(xintercept = detected_cps, color = cp_col_combined, linetype = "solid", size = 1.2)} +
...
# 6. 主程序运行
# ==========================================
run_real_data_eval_2cp <- function(data_dir, N_sim = 5, raw_coords) {
  base_data_list <- load_real_data(data_dir)
  n_len <- nrow(base_data_list[[1]])
  
  # 设置两个真实变点，分布在序列的三分之一和三分之二处
  cp_true <- c(floor(n_len / 3), floor(2 * n_len / 3)) 
  shift_size <- 100 
  ...
  for (cond in conditions) {
    ...
    # 统一前缀为 2CP
    generate_best_plots(best_run, cond, cp_true=cp_true, L=12, M=500, block_size=4, coords=coords, prefix="BestRun_RealData_2CP", n=n_len, p=2)
...
set.seed(2026)
mc_results <- run_real_data_eval_2cp(data_dir = "2025_new", N_sim = 10, raw_coords = raw_station_coords)
