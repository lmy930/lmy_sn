...
# ==========================================
# 3. 双变点方差异常注入 (独立加性噪声 + 绝对值反弹)
# ==========================================
generate_spatial_data_real_2cp_var <- function(base_data_list, cp_locs, noise_sd_1, noise_sd_2, condition) {
  ...
  for (i in seq_len(K)) {
    if (c1 <= n && c2 <= n && c1 < c2) {
      ...
      if ((condition == "Anomaly_Both" && i == 1) || (condition == "Normal") ||
        ...
        data_list[[i]][c1:(c2-1), 1] <- data_list[[i]][c1:(c2-1), 1] + noise_v1
      }
      
      if ((condition == "Anomaly_Both" && i == 1) || (condition == "Normal") ||
          (condition == "Anomaly_Var1") || (condition == "Anomaly_Var2" && i == 1)) {
        data_list[[i]][c1:(c2-1), 2] <- data_list[[i]][c1:(c2-1), 2] + noise_v2
      }
      
      # 绝对值反弹，防止低谷期方差被负数截断抹平
...
# 多变点评估逻辑 (包含双变点惩罚与打分)
evaluate_metrics_2cp <- function(n, cp_true, detected_cps, tol = 12, condition) {
...
# 5. 奇异修复 + 2CP 贪婪选择逻辑
# ==========================================
run_case_logic_real_2cp_var <- function(condition, base_data_list, coords, cp_true, noise_sd_1, noise_sd_2, L=12, M=500, block_size=4) {
  n <- nrow(base_data_list[[1]])
  p <- ncol(base_data_list[[1]])
  
  data_list <- generate_spatial_data_real_2cp_var(base_data_list, cp_true, noise_sd_1, noise_sd_2, condition)
  target_mat <- data_list[[1]]
  
  # Stage 1 专用：强制标准化 + Jitter 防止奇异矩阵报错
  ...
  if (length(candidates) > 0) {
    # 按照 S_obs 差异显著性从大到小排序
    s_obs_vals <- sapply(candidates, function(x) x$S_obs)
    sorted_indices <- order(s_obs_vals, decreasing = TRUE)
    
    # 贪婪搜索：最多挑选两个满足互斥距离 (距离 > L) 的变点
    for (idx in sorted_indices) {
      cand_tau <- candidates[[idx]]$tau
      if (length(final_detected_cps) < 2) {
        if (length(final_detected_cps) == 0 || !any(abs(final_detected_cps - cand_tau) <= L)) {
          final_detected_cps <- c(final_detected_cps, cand_tau)
        }
      }
    }
  }
  
  metrics <- evaluate_metrics_2cp(n, cp_true, final_detected_cps, tol = L, condition)
...
# 7. 主程序入口
# ==========================================
run_real_data_eval_2cp_var <- function(data_dir, N_sim = 5, raw_coords) {
  base_data_list <- load_real_data(data_dir)
  n_len <- nrow(base_data_list[[1]])
  
  # 💡 结合实际数据趋势图的长度 (约 72 个时间点)
  # 将变点设置在平缓期的开头和结尾附近，留出充分的计算窗口
  cp_true_setup <- c(25, 50) 
  ...
  for (cond in conditions) {
    ...
    for (iter in 1:N_sim) {
      res <- run_case_logic_real_2cp_var(cond, base_data_list, coords, cp_true=cp_true_setup, 
      ...
      if(is_anomaly_cond) {
        if(res$metrics$TP == 2) correct_count <- correct_count + 1 # 双变点全找到
    ...
    generate_best_plots_real_2cp_var(best_run, cond, cp_true=cp_true_setup, L=12, M=500, block_size=4, 
                                     coords=coords, prefix="BestRun_RealData_VAR_2CP", n=n_len, p=2)
    
    summary_df <- rbind(summary_df, data.frame(
      Condition = cond,
      Exact_Match_Rate = round(correct_count / N_sim, 3),
...
set.seed(2026)
mc_results_real_2cp_var <- run_real_data_eval_2cp_var(data_dir = "2025_new", N_sim = 30, raw_coords = raw_station_coords)