...
delta1_case1 <- c(0, -8.0)
delta2_case1 <- c(2.0, -2.0)

# 情况2：
# 第1个变点：仅变量1突变（对应原情形3）
# 第2个变点：变量1和变量2同步突变（对应原情形4）
delta1_case2 <- c(-10.0, 0)
delta2_case2 <- c(10.0, -8.0)
...
run_case1_chunk <- function(iter_vec) {
  lapply(iter_vec, run_case1_one)
}

run_case2_chunk <- function(iter_vec) {
  lapply(iter_vec, run_case2_one)
}
...
setup_parallel_cluster <- function(n_cores) {
  ...
  objects_to_export <- c(
    ...
    "delta1_case1", "delta2_case1", "delta1_case2", "delta2_case2",
    "cols",
    "get_default_cores", "split_iterations",
    "rmvnorm2", "simulate_var1_2cp",
    "get_cp", "detect_uni", "detect_multi",
    "detect_indicator", "hausdorff_cp", "cp_to_label", "ari_score", "ari_cp",
    "count_hit_cp", "safe_mean",
    "score_case1", "score_case2",
    "make_df", "plot_result",
    "run_case1_one", "run_case2_one",
    "run_case1_chunk", "run_case2_chunk"
  )
...
if (!is.null(cl)) {
  ...
  case1_chunks <- parallel::parLapplyLB(
    cl,
    X = iter_chunks,
    fun = run_case1_chunk
  )
  
  case1_list <- unlist(case1_chunks, recursive = FALSE, use.names = FALSE)
} else {
  case1_list <- lapply(seq_len(B), run_case1_one)
}
...
if (!is.null(cl)) {
  ...
  case2_chunks <- parallel::parLapplyLB(
    cl,
    X = iter_chunks,
    fun = run_case2_chunk
  )
  
  case2_list <- unlist(case2_chunks, recursive = FALSE, use.names = FALSE)
} else {
  case2_list <- lapply(seq_len(B), run_case2_one)
}
